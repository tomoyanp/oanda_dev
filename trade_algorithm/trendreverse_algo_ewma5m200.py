            ...
            if 1 == 1:
                if seconds < 10:
                    if self.first_trade_flag == "pass":
                        if self.slope_1m > 0 and current_price > self.ewma20_5mvalue and (current_price - 0.01) < self.ewma20_1mvalue < current_price and (self.ask_price - self.bid_price) < 0.01 and (self.upper_sigma_5m2 - self.lower_sigma_5m2) > 0.1 and current_price > self.ewma200_5mvalue:
                            trade_flag = "buy"
                            self.algorithm = "cross over base_line"
                            #self.first_trade_flag = "buy"
                            #self.first_trade_time = base_time
    
                        elif self.slope_1m < 0 and current_price < self.ewma20_5mvalue and current_price < self.ewma20_1mvalue < (current_price + 0.01) and (self.ask_price - self.bid_price) < 0.01 and (self.upper_sigma_5m2 - self.lower_sigma_5m2) > 0.1 and current_price < self.ewma200_5mvalue:
                            trade_flag = "sell"
                            self.algorithm = "cross over base_line"
...
#        self.ewma20_1mvalue = getEWMA(ewma20_rawdata, len(ewma20_rawdata))[-1]
        ...
        for res in response:
            tmp.append(res[0])
        tmp.reverse()
        self.ewma200_5mvalue = getEWMA(tmp, len(tmp))[-1]
